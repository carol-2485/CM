...
class _CentroDoacao {
  ...
  _CentroDoacao({required this.id, required this.nome, required this.morada,
...
class _CentrosScreenState extends State<CentrosScreen> {
  ...
  static final List<_CentroDoacao> _centrosPadrao = [
    _CentroDoacao(id: 'sete_rios', nome: 'Centro de Saúde de Sete Rios',
        morada: 'Rua Prof. Lima Basto, Lisboa', latitude: 38.7436, longitude: -9.1614),
    _CentroDoacao(id: 'santa_maria', nome: 'Hospital de Santa Maria',
        morada: 'Av. Prof. Egas Moniz, Lisboa', latitude: 38.7480, longitude: -9.1599),
    _CentroDoacao(id: 'lusiadas', nome: 'Hospital dos Lusíadas',
        morada: 'Rua Abílio Torres, Lisboa', latitude: 38.7380, longitude: -9.1700),
    _CentroDoacao(id: 'luz', nome: 'Hospital da Luz',
        morada: 'Av. Lusíada, Lisboa', latitude: 38.7543, longitude: -9.1952),
    _CentroDoacao(id: 'ipst', nome: 'Instituto Português do Sangue',
        morada: 'Av. Miguel Bombarda, Lisboa', latitude: 38.7297, longitude: -9.1476),
  ];
  ...
  Future<void> _carregarCentros() async {
    try {
      final resultado = await FirebaseFirestore.instance.collection('centros').get();
    ...
    } catch (_) { _centros = _centrosPadrao; }
  }
  ...
  Future<void> _obterLocalizacao() async {
    try {
      ...
      final posicao = await Geolocator.getCurrentPosition(
        ...
        onTimeout: () => Position(
          latitude: _centroLisboa.latitude, longitude: _centroLisboa.longitude,
          timestamp: DateTime.now(), accuracy: 0, altitude: 0, altitudeAccuracy: 0,
          heading: 0, headingAccuracy: 0, speed: 0, speedAccuracy: 0,
        ),
      );

      final distancia = _haversine(posicao.latitude, posicao.longitude,
  ...
  void _ordenarPorProximidade() {
    for (final centro in _centros) {
      centro.distancia = _haversine(_posicaoUtilizador.latitude, _posicaoUtilizador.longitude,
  ...
  void _seleccionarCentro(_CentroDoacao centro) {
    ...
    _controladorMapa.move(LatLng(centro.latitude, centro.longitude), 14);
  }
      ...
      body: _aCarregar
          ...
          : Column(children: [
              ...
              _BotaoConfirmar(
                ...
                aoConfirmar: () => Navigator.pushNamed(context, AppRoutesUser.agenda,
                    arguments: {'centroId': _centroSeleccionado!.id, 'centroNome': _centroSeleccionado!.nome}),
              ),
            ]),
...
class _MapaCentros extends StatelessWidget {
  ...
  const _MapaCentros({required this.centros, required this.centroSeleccionado,
      required this.controladorMapa, required this.posicaoUtilizador, required this.aoSeleccionar});
        ...
        child: FlutterMap(
          ...
          options: MapOptions(
            initialCenter: centroSeleccionado != null
                ? LatLng(centroSeleccionado!.latitude, centroSeleccionado!.longitude)
                : posicaoUtilizador,
            initialZoom: 12,
          ),
          children: [
            ...
            MarkerLayer(
              markers: centros.map((c) {
                ...
                return Marker(
                  ...
                  width: seleccionado ? 48 : 38, height: seleccionado ? 48 : 38,
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.primary, width: seleccionado ? 3 : 2),
        boxShadow: seleccionado
            ? [BoxShadow(color: AppColors.primary.withValues(alpha: 0.4), blurRadius: 8)]
            : null,
      ),
      child: Icon(Icons.local_hospital_rounded,
          color: seleccionado ? Colors.white : AppColors.primary, size: seleccionado ? 26 : 20),
    );
  }
}
...
class _ListaCentros extends StatelessWidget {
  ...
  const _ListaCentros({required this.centros, required this.centroSeleccionado, required this.aoSeleccionar});
...
class _TileCentro extends StatelessWidget {
  ...
  const _TileCentro({required this.centro, required this.seleccionado, required this.aoSeleccionar});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: seleccionado ? AppColors.primary.withValues(alpha: 0.07) : AppColors.surface,
        ...
        child: Row(children: [
          AnimatedContainer(
            ...
            width: 38, height: 38,
            decoration: BoxDecoration(
              color: seleccionado ? AppColors.primary.withValues(alpha: 0.12) : AppColors.background,
              borderRadius: BorderRadius.circular(10),
            ),
            child: Icon(Icons.local_hospital_rounded,
                color: seleccionado ? AppColors.primary : AppColors.textMuted, size: 20),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(centro.nome, style: TextStyle(
                fontSize: 13, fontWeight: FontWeight.w600,
                color: seleccionado ? AppColors.primary : AppColors.accent)),
            Text(centro.morada, style: const TextStyle(fontSize: 11, color: AppColors.textMuted)),
          ])),
          Text(centro.distanciaFormatada, style: TextStyle(
              fontSize: 12, fontWeight: FontWeight.w600,
              color: seleccionado ? AppColors.primary : AppColors.textMuted)),
          const SizedBox(width: 4),
          Icon(seleccionado ? Icons.check_circle_rounded : Icons.chevron_right_rounded,
              color: seleccionado ? AppColors.primary : AppColors.textMuted, size: 18),
        ]),
      ),
    );
  }
