...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  String? _temHistorico;
  String _textoHistorico = '';

  @override
  void dispose() {
    _ctrlNome.dispose(); _ctrlIdade.dispose(); _ctrlSangue.dispose();
    _ctrlHistoricoTexto.dispose(); _ctrlData.dispose();
    _ctrlEmail.dispose(); _ctrlPassword.dispose();
    super.dispose();
  }
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                _CampoTexto(ctrl: _ctrlNome, dica: 'Nome*', icone: Icons.person_outline),
                _CampoTexto(ctrl: _ctrlIdade, dica: 'Idade*', icone: Icons.cake_outlined, teclado: TextInputType.number),
                _CampoTexto(ctrl: _ctrlSangue, dica: 'Tipo Sanguíneo*', icone: Icons.water_drop_outlined),
                _CampoHistorico(
                  valorSelecionado: _temHistorico,
                  onAlterado: (v) => setState(() { _temHistorico = v; if (v == 'nao') _textoHistorico = ''; }),
                ...
                _CampoTexto(
                  ctrl: _ctrlEmail, dica: 'Email*', icone: Icons.alternate_email,
                ...
                _CampoPassword(
                  ctrl: _ctrlPassword, esconder: _esconderPassword,
                  onAlternarVisibilidade: () => setState(() => _esconderPassword = !_esconderPassword),
                ),
                _BotaoCriarConta(aRegistar: _aRegistar, aoPremir: _registar),
...
class _CampoTexto extends StatelessWidget {
  ...
  const _CampoTexto({required this.ctrl, required this.dica, required this.icone,
      this.teclado = TextInputType.text, this.validador});
  @override
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: TextFormField(
        ...
        validator: validador ?? (v) => v == null || v.isEmpty ? 'Campo obrigatório' : null,
      ),
    );
  }
}
...
class _CampoHistorico extends StatelessWidget {
  ...
  const _CampoHistorico({required this.valorSelecionado, required this.onAlterado,
      required this.mostrarCampoDescricao, required this.onDescricaoAlterada, required this.ctrlDescricao});
  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: DropdownButtonFormField<String>(
            ...
            decoration: _decoracao('Histórico de doenças*', Icons.medical_information_outlined),
            items: const [
              DropdownMenuItem(value: 'nao', child: Text('Não')),
              DropdownMenuItem(value: 'sim', child: Text('Sim, qual?')),
            ],
        ...
        if (mostrarCampoDescricao) ...[
          ...
          TextFormField(
            controller: ctrlDescricao, onChanged: onDescricaoAlterada,
            decoration: _decoracaoArredondada('Descreva brevemente*', Icons.edit_note_rounded),
            validator: (v) => (v == null || v.isEmpty) ? 'Descreva o histórico' : null,
          ),
        ],
      ]),
    );
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        onTap: () async {
          ...
          if (escolhida != null) {
            ctrl.text = '${escolhida.day.toString().padLeft(2,'0')}/${escolhida.month.toString().padLeft(2,'0')}/${escolhida.year}';
          }
        },
        decoration: _decoracao('Data da última doação', Icons.calendar_month_outlined),
      ),
    );
  }
}
...
class _CampoPassword extends StatelessWidget {
  ...
  const _CampoPassword({required this.ctrl, required this.esconder, required this.onAlternarVisibilidade});
  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 24),
      child: TextFormField(
        controller: ctrl, obscureText: esconder,
        decoration: _decoracao('Password*', Icons.lock_outline).copyWith(
          suffixIcon: IconButton(
            icon: Icon(esconder ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: AppColors.textMuted),
...
class _BotaoCriarConta extends StatelessWidget {
  ...
  const _BotaoCriarConta({required this.aRegistar, required this.aoPremir});
  @override
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: aRegistar
            ...
            : const Text('Criar conta', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white)),
      ),
    );
  }
}
